...
<div class="space-y-6 animate-fade-in max-w-2xl mx-auto">
    ...
    <div class="bg-white p-5 rounded-xl shadow-card border border-gray-100 flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Edit Data Kecamatan</h1>
            <p class="text-sm text-gray-500 mt-1">{{ $kecamatan->nama }} ({{ $kecamatan->kode }})</p>
        </div>
    </div>

    <div class="bg-white rounded-xl shadow-card border border-gray-100 overflow-hidden">
        <form action="{{ route('kecamatan.update', $kecamatan) }}" method="POST" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="kode" class="form-input w-full" value="{{ old('kode', $kecamatan->kode) }}" required>
                ...
                <div>
                    ...
                    <input type="text" name="nama" class="form-input w-full" value="{{ old('nama', $kecamatan->nama) }}" required>
                    @error('nama') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>
                <div class="md:col-span-2">
                    <label class="form-label block text-sm font-medium text-gray-700 mb-1">Nama Camat</label>
                    <input type="text" name="camat" class="form-input w-full" value="{{ old('camat', $kecamatan->camat) }}">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="telepon" class="form-input w-full" value="{{ old('telepon', $kecamatan->telepon) }}">
            ...
            <div>
                ...
                <textarea name="alamat" rows="2" class="form-input w-full">{{ old('alamat', $kecamatan->alamat) }}</textarea>
